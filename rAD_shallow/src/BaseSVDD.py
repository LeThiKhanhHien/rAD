...
class BaseSVDD(BaseEstimator, OutlierMixin):
    ...
    def fit(self, X, y=None, weight=None):
        ...
        self.accuracy = accuracy_score(np.array(self.y), np.array(self.predicted_y_))
    ...
    def predict(self, X, y=None):
        ...
        if results['exist_y'] == True:       
            results['accuracy'] = accuracy_score(np.array(results['y']), np.array(results['predicted_y']))
